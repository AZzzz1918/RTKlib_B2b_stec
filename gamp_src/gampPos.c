...
static void outiPppHead(FILE *fp, rtk_t rtk)
{
	...
	if (!obss.data[ig].type[0]) obss.data[ig].type[0]="  ";
	if (!obss.data[ig].type[1]) obss.data[ig].type[1]="  ";
	if (!obss.data[ig].type[2]) obss.data[ig].type[2]="  ";
	if (!obss.data[ir].type[0]) obss.data[ir].type[0]="  ";
	if (!obss.data[ir].type[1]) obss.data[ir].type[1]="  ";
	if (!obss.data[ic].type[0]) obss.data[ic].type[0]="  ";
	if (!obss.data[ic].type[1]) obss.data[ic].type[1]="  ";
	if (!obss.data[ic].type[2]) obss.data[ic].type[2]="  ";
	if (!obss.data[ie].type[0]) obss.data[ie].type[0]="  ";
	if (!obss.data[ie].type[1]) obss.data[ie].type[1]="  ";
	if (!obss.data[ie].type[2]) obss.data[ie].type[2]="  ";
...
static int rtkpos(rtk_t *rtk, obsd_t *obs, int n, const nav_t *nav)
{
	...
	printf("DEBUG: nu after obsScan_PPP = %d\n", nu);
	               ...
	               const solopt_t *sopt, filopt_t *fopt)
{
	...
	if (PMODE_PPP_KINEMA<=popt->mode) {
		...
		if (popt->navsys&SYS_GLO) setcodepri(SYS_GLO,1,popt->sateph==EPHOPT_PREC?"PC":"PC");
		if (popt->navsys&SYS_GAL) setcodepri(SYS_GAL,1,popt->sateph==EPHOPT_PREC?"CABXZ":"CABXZ");
		if (popt->navsys&SYS_CMP) setcodepri(SYS_CMP,1,popt->sateph==EPHOPT_PREC?"IQX":"IQX");
	}
	...
	if (*fopt->mgexdcbf&&(popt->navsys&SYS_CMP||popt->navsys&SYS_GAL||popt->navsys&SYS_GLO))
