...
extern void obsScan_PPP(const prcopt_t *popt, obsd_t *obs, const int nobs, int *nValid)
{
	...
	for (i=n=0;i<nobs&&i<MAXOBS;i++) {
		...
		f2=1;
		if (popt->nf>=3&&(PPP_Glo.sFlag[sat-1].sys&(SYS_GAL|SYS_CMP|SYS_SBS|SYS_GPS))) f2=2;
		
                if (popt->mode>=PMODE_PPP_KINEMA) {
                    if (sys==SYS_GLO) {
                        printf("GLONASS sat=%d L[0]=%f L[1]=%f P[0]=%f P[1]=%f\n", sat, obs[i].L[0], obs[i].L[1], obs[i].P[0], obs[i].P[1]);
                    }
                    else if (sys==SYS_GAL) {
                        printf("Galileo sat=%d L[0]=%f L[1]=%f P[0]=%f P[1]=%f\n", sat, obs[i].L[0], obs[i].L[1], obs[i].P[0], obs[i].P[1]);
                    }
                    else if (sys==SYS_CMP) {
                        printf("BeiDou sat=%d L[0]=%f L[1]=%f P[0]=%f P[1]=%f\n", sat, obs[i].L[0], obs[i].L[1], obs[i].P[0], obs[i].P[1]);
                    }
                    else if (sys==SYS_GPS) {
                        printf("GPS sat=%d L[0]=%f L[1]=%f P[0]=%f P[1]=%f\n", sat, obs[i].L[0], obs[i].L[1], obs[i].P[0], obs[i].P[1]);
                    }

			if (obs[i].L[0]*obs[i].L[f2]==0.0) {
				printf("SKIP: sat=%d sys=%d L[0]=%f L[%d]=%f\n", sat, sys, obs[i].L[0], f2, obs[i].L[f2]);
				continue;
			}
		}

		if (fabs(obs[i].P[0]-obs[i].P[f2])>=200.0) {
			printf("SKIP: sat=%d sys=%d P[0]=%f P[%d]=%f diff=%f\n", sat, sys, obs[i].P[0], f2, obs[i].P[f2], fabs(obs[i].P[0]-obs[i].P[f2]));
			continue;
		}
