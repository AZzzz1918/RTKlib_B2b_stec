...
static void udclk_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
	...
	for (i=1;i<NSYS;i++) {
		...
		if (sys&SYS_GLO&&i==1) {
			if (opt->navsys == SYS_GLO) continue; // single GLO mode
...
static void udiono_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
	...
	for (i=0;i<MAXSAT;i++) {
		...
		if (rtk->x[j]!=0.0&&(int)rtk->ssat[i].outc[0]>gap_resion) {
			if ((i+1)==11||(i+1)==22) {
				sprintf(PPP_Glo.chMsg,"*** DEBUG IONO RESET: sat=%d (%s) outc=%d x=%.4f\n",
					i+1, PPP_Glo.sFlag[i].id, rtk->ssat[i].outc[0], rtk->x[j]);
				outDebug(OUTWIN,OUTFIL,OUTTIM);
			}
			rtk->x[j]=0.0;
		}
	...
	for (i=0;i<n;i++) {
		...
		PPP_Glo.prcOpt_Ex.ionopnoise==2)&&rtk->x[j]==0.0)) {
			if ((sat==11||sat==22)&&rtk->x[j]==0.0) {
				sprintf(PPP_Glo.chMsg,"*** DEBUG IONO INIT: sat=%d (%s) ion_const=%d elev=%.1f\n",
					sat, PPP_Glo.sFlag[sat-1].id, PPP_Glo.prcOpt_Ex.ion_const, elev);
				outDebug(OUTWIN,OUTFIL,OUTTIM);
			}
...
static void udbias_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
	...
	for (f=0;f<NF(&rtk->opt);f++) {
		...
		for (i=0;i<MAXSAT;i++) {
			if (++rtk->ssat[i].outc[f]>(unsigned int)rtk->opt.maxout||
				rtk->opt.modear==ARMODE_INST||clk_jump) {
					if ((i+1)==11||(i+1)==22) {
						sprintf(PPP_Glo.chMsg,"*** DEBUG BIAS RESET: sat=%d (%s) f=%d outc=%d maxout=%d\n",
							i+1, PPP_Glo.sFlag[i].id, f, rtk->ssat[i].outc[f], rtk->opt.maxout);
						outDebug(OUTWIN,OUTFIL,OUTTIM);
					}
		...
		for (i=k=0;i<n&&i<MAXOBS;i++) {
			...
			bias[i]=0.0;
			// sat=%d ionoopt=%d IONOOPT_IF12=%d Lc=%f Pc=%f f=%d\n", sat, rtk->opt.ionoopt, IONOOPT_IF12, Lc, Pc, f);
			...
			if (rtk->opt.ionoopt==IONOOPT_IF12) {
				...
				l=1;
				lam=nav->lam[sat-1];
				//if (satsys(sat,NULL)==SYS_GLO) // sat=%d P0=%f P1=%f lam0=%f lam1=%f lamf=%f f=%d\n", sat, obs[i].P[0], obs[i].P[1], lam[0], lam[1], lam[f], f);
				if (obs[i].P[0]==0.0||obs[i].P[l]==0.0||
					lam[0]==0.0||lam[l]==0.0||lam[f]==0.0) {
					if (sat==11||sat==22) {
						sprintf(PPP_Glo.chMsg,"*** DEBUG BIAS SKIP: sat=%d (%s) f=%d P0=%.3f P1=%.3f lam0=%.6f lam1=%.6f lamf=%.6f\n",
							sat, PPP_Glo.sFlag[sat-1].id, f, obs[i].P[0], obs[i].P[l], lam[0], lam[l], lam[f]);
						outDebug(OUTWIN,OUTFIL,OUTTIM);
					}
					continue;
				}
			...
			if ((sat==11||sat==22)&&bias[i]==0.0) {
				sprintf(PPP_Glo.chMsg,"*** DEBUG BIAS SKIP2: sat=%d (%s) f=%d bias=0 L=%.4f,%.4f P=%.4f,%.4f\n",
					sat, PPP_Glo.sFlag[sat-1].id, f, L[0], L[1], P[0], P[1]);
				outDebug(OUTWIN,OUTFIL,OUTTIM);
			}
			continue;
		}
		...
		for (i=0;i<n&&i<MAXOBS;i++) {
			...
			//if (satsys(sat,NULL)==SYS_GLO) printf("initx sat=%d f=%d bias=%f\n", sat, f, bias[i]);
			if (sat==11||sat==22) {
				sprintf(PPP_Glo.chMsg,"*** DEBUG BIAS INIT: sat=%d (%s) f=%d bias=%.4f L[0]=%.4f P[0]=%.4f\n",
					sat, PPP_Glo.sFlag[sat-1].id, f, bias[i], L[0], P[0]);
				outDebug(OUTWIN,OUTFIL,OUTTIM);
			}
	...
	double *v, double *H, double *R, double *azel)
{
	...
	printf("ppp_res entry: post=%d n=%d norm(rr,3)=%f\n", post, n, norm(rr,3));
	if (norm(rr,3)<=100.0) {
		printf("ppp_res failed because norm(rr,3)<=100! rr=%f %f %f\n", rr[0], rr[1], rr[2]);
		return 0;
	}
	...
	for (i=0;i<n&&i<MAXOBS;i++) {
		...
		if (lam[j/2]==0.0||lam[0]==0.0) {
                                                printf("SKIP lam=0 sat=%d lam0=%f lam1=%f\n", sat, lam[0], lam[1]); continue;
                }
                if ((r=geodist(rs+i*6,rr,e))<=0.0||
                        satazel(pos,e,azel+i*2)<opt->elmin) {
				exc[i]=1; printf("SKIP azel or r=0 sat=%d r=%f el=%f elmin=%f\n", sat, r, satazel(pos,e,azel+i*2), opt->elmin); continue;
		}
		if (!(sys=PPP_Glo.sFlag[sat-1].sys)||
			satexclude(obs[i].sat,svh[i],opt)||exc[i]) {
                                                printf("SKIP excl sat=%d sys=%d exc=%d svh=%d excluded=%d\n", sat, sys, exc[i], svh[i], satexclude(obs[i].sat,svh[i],opt)); continue;
                }
		...
		for (j=0;j<2*NF(opt);j++) {
			...
			if (opt->ionoopt==IONOOPT_IF12) {
				if ((y=j%2==0?Lc:Pc)==0.0) { printf("SKIP y=0 sat=%d\n", sat); continue;}
			}
			else {
				if ((y=j%2==0?L[j/2]:P[j/2])==0.0) {
					printf("SKIP_Y y=0 sat=%d frq=%d\n", sat, j);
					continue;
				}
			}
			...
			if (sys==SYS_GLO) {
				...
				else if (opt->gloicb==GLOICB_OFF||opt->gloicb==GLOICB_LNF||opt->gloicb==GLOICB_QUAD) {  //handling of ISBs
					ic=IC(0,opt);
					id=IC(1,opt);
					cdtr=x[ic]+x[id];
					H[ic+nx_nv]=1.0;
			...
			if (opt->ionoopt==IONOOPT_UC1||opt->ionoopt==IONOOPT_UC12) {
				...
				if (x[ic]==0.0) { 
					if (sat==11||sat==22) {
						sprintf(PPP_Glo.chMsg,"*** DEBUG: SKIP xic=0 sat=%d (%s) j=%d\n", sat, PPP_Glo.sFlag[sat-1].id, j);
						outDebug(OUTWIN,OUTFIL,OUTTIM);
					}
					continue; 
				}
				...
				//	}
				//}
				if (PPP_Glo.prcOpt_Ex.ion_const&&(j/2==1&&j%2==1)) {
                                        int sys_idx = 0;
                                        if (sys==SYS_GLO) sys_idx = 1;
                                        else if (sys==SYS_CMP) sys_idx = 2;
                                        else if (sys==SYS_GAL) sys_idx = 3;
                                        else if (sys==SYS_QZS) sys_idx = 4;
                                        dcb+=rtk->x[ID(sys_idx, opt)];
                                        H[ID(sys_idx, opt)+nx_nv]=1.0;
                                }
			}
			...
			if (j%2==0) {  /* phase bias */
				...
				if ((bias=x[ic])==0.0) { 
					if (sat==11||sat==22) {
						sprintf(PPP_Glo.chMsg,"*** DEBUG: SKIP bias=0 sat=%d (%s) j=%d\n", sat, PPP_Glo.sFlag[sat-1].id, j);
						outDebug(OUTWIN,OUTFIL,OUTTIM);
					}
					continue; 
				}
			...
			if (j%2==0&&(sat==11||sat==22)) {
				sprintf(PPP_Glo.chMsg,"*** DEBUG: vsat[%d] SET to 1 for sat=%d (%s)\n",
					j/2, sat, PPP_Glo.sFlag[sat-1].id);
				outDebug(OUTWIN,OUTFIL,OUTTIM);
			}
	...
	printf("ppp_res return: nv=%d stat=%d\n", nv, stat);
...
static void update_stat(rtk_t *rtk, const obsd_t *obs, int n, int stat)
{
	...
	rtk->sol.dcb_rcv=rtk->x[ID(0, opt)];
