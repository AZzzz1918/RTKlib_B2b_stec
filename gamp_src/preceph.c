                    ...
                    double *bfact, char *tsys)
{
    ...
    for (i=0;i<100;i++) {
        ...
        else if (buff[0]=='%'&&buff[1]=='c') {
            if (tsys[0]=='\0') { strncpy(tsys,buff+9,3); tsys[3]='\0'; }
        }
        else if (buff[0]=='%'&&buff[1]=='f') {
            if (bfact[0]==0.0) {
...
extern int readdcb_mgex(const char *file, nav_t *nav, const gtime_t time)
{
	...
	while (fgets(buff,sizeof(buff),fp)) {
		...
		if (sys == SYS_GPS) {
			...
			if (!strncmp(buff+25,"C1C  C2C",8) || !strncmp(buff+25,"C1P  C2P",8)) {
				cbias = str2num(buff,71,21);
				nav->cbias[sat-1][0] = cbias*1E-9*CLIGHT; /* ns -> m */
			}
			else if (!strncmp(buff+25,"C1C  C1P",8)) {
				cbias = str2num(buff,71,21);
				nav->cbias[sat-1][1] = cbias*1E-9*CLIGHT; /* ns -> m */
			}
			else if (!strncmp(buff+25,"C2C  C2P",8)) {
				cbias = str2num(buff,71,21);
				nav->cbias[sat-1][2] = cbias*1E-9*CLIGHT; /* ns -> m */
			}
		}
	}
	fclose(fp);
