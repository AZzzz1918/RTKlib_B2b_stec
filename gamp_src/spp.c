				   ...
				   double *v, double *H, double *var, double *azel, int *vsat, double *resp, int *nx, int *bDeleted)
{
    ...
    *nx=NX_SPP;
	...
	printf("DEBUG: processing obs for sys %d sat=%d\n", sys, sat);
        ...
        if ((r=geodist(rs+i*6,rr,e))<=0.0) { printf("DEBUG: geodist r<=0 sat=%d\n", sat); continue; }
        ...
        if ((P=prange(obs+i,nav,azel+i*2,opt,&vmeas))==0.0) { printf("DEBUG: prange P==0 sat=%d\n", sat); continue; }
        ...
        if (sys==SYS_GLO) {
            if (opt->navsys == SYS_GLO) {
                H[4+nv*NX_SPP]=H[5+nv*NX_SPP]=0.0; ns[0]++; bObserved[0]=1;
            } else {
                v[nv]-=x[4];   H[4+nv*NX_SPP]=1.0; ns[1]++; bObserved[1]=1;
            }
        }
