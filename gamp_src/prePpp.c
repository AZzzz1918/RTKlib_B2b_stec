	...
	const prcopt_t popt, const solopt_t sopt, filopt_t *fopt)
{
	...
	ni+=n;
	printf("*** DEBUG: NAV FILES FOUND = %d, num paths = %d, path: %s\n", n, ni, fopt->inf[ni-1]);
	...
	findP2C2DcbFile(ts,te,dir,fopt->p2c2dcbf);
        if (popt.navsys&SYS_CMP||popt.navsys&SYS_GAL||popt.navsys&SYS_GLO) findMGEXDcbFile(ts, te, dir, fopt->mgexdcbf);
...
extern void batchProc(char folder[], char cfgfile[])
{
	...
	do {
		if (strncmp(file.name,".",1)==0) continue;
		if (!(ext=strrchr(file.name,'.'))) continue;
		if (!strstr(ext+3,"o")) continue;
		sprintf(filepath,"%s%c%s",folder,sep,file.name);
