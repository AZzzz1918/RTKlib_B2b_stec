...
static int readdcbf(const char *file, nav_t *nav, const sta_t *sta)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (sta&&(!strcmp(str1,"G")||!strcmp(str1,"R"))) { /* receiver DCB */
            ...
            nav->legacy_cbias[sat-1][type-1]=cbias*1E-9*CLIGHT; /* ns -> m */
            if (type==2) nav->cbias[sat-1][0][0]=nav->legacy_cbias[sat-1][type-1];
            if (type==3) nav->cbias[sat-1][1][0]=nav->legacy_cbias[sat-1][type-1];
        }
    }
    fclose(fp);
...
extern int readdcb(const char *file, nav_t *nav, const sta_t *sta)
{
    ...
    for (i=0;i<MAXSAT;i++) for (j=0;j<3;j++) nav->legacy_cbias[i][j]=0.0;
    for (i=0;i<MAXEXFILE;i++) {
        if (!(efiles[i]=(char *)malloc(1024))) {
            for (i--;i>=0;i--) free(efiles[i]);
            return 0;
...
{
    // pco的读取和你设置的卫星系统无关，
    // 但是nav里的读取要求必须只读取指定卫星的，例如要是只设置了BDS，这里就不会改GPS!!
    ...
    cross3(ey,ez,ex);
    trace(2, "satantoff2: Cross product of ey and ez (ex): %.3f %.3f %.3f\n", ex[0], ex[1], ex[2]);
    ...
    trace(2, "satantoff2: Frequencies selected: freq[0]=%.6f freq[1]=%.6f\n", freq[0], freq[1]);

    
    C1= SQR(freq[0])/(SQR(freq[0])-SQR(freq[1]));
    C2=-SQR(freq[1])/(SQR(freq[0])-SQR(freq[1]));
    trace(2, "satantoff2: pcv->off[fi]: %.3f %.3f %.3f\n", pcv->off[fi][0], pcv->off[fi][1], pcv->off[fi][2]);
    trace(2, "satantoff2: pcv->off[fj]: %.3f %.3f %.3f\n", pcv->off[fj][0], pcv->off[fj][1], pcv->off[fj][2]);
    

    /* iono-free LC */
    for (i=0;i<3;i++) {
        if(sys==SYS_CMP&&B2bAPC_FLAG) {
            dant[i]=pcv->off[fj][0]*ex[i]+pcv->off[fj][1]*ey[i]+pcv->off[fj][2]*ez[i];
            trace(2, "satantoff2: CMP system with B2bAPC_FLAG: dant[%d] = %.3f\n", i, dant[i]);
            }
        else{
            dant1=pcv->off[fi][0]*ex[i]+pcv->off[fi][1]*ey[i]+pcv->off[fi][2]*ez[i];
            dant2=pcv->off[fj][0]*ex[i]+pcv->off[fj][1]*ey[i]+pcv->off[fj][2]*ez[i];
            dant[i]=C1*dant1+C2*dant2;
            trace(7, "satantoff2: Computed dant[%d] = %.3f (dant1=%.3f, dant2=%.3f)\n", 
                  i, dant[i], dant1, dant2);
            }
    }
}
	...
	double *dantf1, double *dantf2)
{
	...
	dantf1[0] = dantf1[1] = dantf1[2] = 0.;
	dantf2[0] = dantf2[1] = dantf2[2] = 0.;
