                      ...
                      char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
    ...
    /* assign index for highest priority code by observation type, as in GAMP */
    for (i=0;i<NFREQ;i++) {
        k0=k1=k2=k3=-1;
        for (j=0;j<n;j++) {
            if (ind->idx[j]!=i||!ind->pri[j]) continue;
            if (ind->type[j]==0) {
                if (k0<0||ind->pri[j]>ind->pri[k0]) k0=j;
            }
            else if (ind->type[j]==1) {
                if (k1<0||ind->pri[j]>ind->pri[k1]) k1=j;
            }
            else if (ind->type[j]==2) {
                if (k2<0||ind->pri[j]>ind->pri[k2]) k2=j;
            }
            else if (ind->type[j]==3) {
                if (k3<0||ind->pri[j]>ind->pri[k3]) k3=j;
            }
        }
    ...
    for (i=0;i<NEXOBS;i++) {
        ...
        for (k=0;k<n;k++) {
            if (ind->code[k]==ind->code[j]&&ind->pos[k]<0) ind->pos[k]=NFREQ+i;
        }
    }
