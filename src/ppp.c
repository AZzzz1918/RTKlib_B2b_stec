...
#define ND(opt)     (((opt)->nf>=3||(opt)->ionoopt==IONOOPT_EST)?NC(opt):0)
#define NT(opt)     ((opt)->tropopt<TROPOPT_EST?0:((opt)->tropopt==TROPOPT_EST?1:3))
#define NI(opt)     ((opt)->ionoopt==IONOOPT_EST?MAXSAT:0)
#define NR(opt)     (NP(opt)+NC(opt)+ND(opt)+NT(opt)+NI(opt))
#define NB(opt)     (NF(opt)*MAXSAT)
#define NX(opt)     (NR(opt)+NB(opt))
#define IC(s,opt)   (NP(opt)+(s))
#define ID(s,opt)   (NP(opt)+NC(opt)+(s))
#define IT(opt)     (NP(opt)+NC(opt)+ND(opt))
#define II(s,opt)   (NP(opt)+NC(opt)+ND(opt)+NT(opt)+(s)-1)
...
static int optflag_int(const prcopt_t *opt, const char *key, int def)
{
    char *p;
    int val=def;

    if (!opt||!key||(p=strstr(opt->pppopt,key))==NULL) return def;
    if (sscanf(p+strlen(key),"=%d",&val)==1) return val;
    return def;
}
static double optflag_double(const prcopt_t *opt, const char *key, double def)
{
    char *p;
    double val=def;

    if (!opt||!key||(p=strstr(opt->pppopt,key))==NULL) return def;
    if (sscanf(p+strlen(key),"=%lf",&val)==1) return val;
    return def;
}
static int gamp_ion_const(const prcopt_t *opt)
{
    return optflag_int(opt,"-ION_CONST",0);
}
static int gamp_ionopnoise(const prcopt_t *opt)
{
    return optflag_int(opt,"-IONOPNOISE",1);
}
static int gamp_gnsisb(const prcopt_t *opt)
{
    return optflag_int(opt,"-GNSISB",1);
}
static double satwavelen(int sat, unsigned char code, const nav_t *nav)
{
    double freq=sat2freq(sat,code,nav);

    return freq>0.0?CLIGHT/freq:0.0;
}
static int sys_index(int sys)
{
    switch (sys) {
    case SYS_GLO: return 1;
    case SYS_CMP: return 2;
...
static double iontec_state_scale(int sat, unsigned char code, const nav_t *nav)
{
    double freq=sat2freq(sat,code,nav);

    if (freq<=0.0) {
        switch (satsys(sat,NULL)) {
        case SYS_CMP: freq=FREQ1_CMP; break;
        default:      freq=FREQL1;    break;
        }
    }
    return SQR(FREQL1/freq);
}

static double ion_gf[MAXSAT]={0};
                     ...
                     int f, const prcopt_t *opt, const obsd_t *obs)
{
    ...
    if (optflag_int(opt,"-GAMP_VARERR",0)) {
        double c=code?opt->eratio[0]:1.0;

        fact=EFACT_GPS;
        if (code&&sys==SYS_GLO) {
            c=optflag_double(opt,"-ERATIO_GLO",opt->eratio[0]);
        }
        else if (code&&sys==SYS_CMP) {
            c=optflag_double(opt,"-ERATIO_BDS",opt->eratio[0]);
        }
        else if (code&&sys==SYS_GAL) {
            c=optflag_double(opt,"-ERATIO_GAL",opt->eratio[0]);
        }
        else if (code&&sys==SYS_QZS) {
            c=optflag_double(opt,"-ERATIO_QZS",opt->eratio[0]);
        }
        if (opt->ionoopt==IONOOPT_IFLC) fact*=3.0;
        return SQR(fact*c)*(SQR(opt->err[1])+SQR(opt->err[2]/sinel));
    }

    /* increase variance for pseudoranges */
    if (code) fact=opt->eratio[frq];
    if (code&&sys==SYS_CMP) {
        double eratio_bds=optflag_double(opt,"-ERATIO_BDS",0.0);
        if (eratio_bds>0.0) fact=eratio_bds;
    }
...
/* L1/L2 geometry-free phase measurement in meters ---------------------------*/
static double gfmeas_new(const obsd_t *obs, const nav_t *nav)
{
    double freq1,freq2,lam1,lam2;
    ...
    lam1=CLIGHT/freq1;
    lam2=CLIGHT/freq2;
    return lam1*obs->L[0]-lam2*obs->L[1];
}
static double gamp_gf_threshold(const rtk_t *rtk)
{
    return rtk->opt.thresslip>0.0?rtk->opt.thresslip:0.15;
}
                      ...
                      double *Lc, double *Pc)
{
    ...
    int i,ix=0,frq,frq2,bias_ix,sys=satsys(obs->sat,NULL),legacy_dcb;

    legacy_dcb=nav->legacy_cbias[obs->sat-1][0]!=0.0||
               nav->legacy_cbias[obs->sat-1][1]!=0.0||
               nav->legacy_cbias[obs->sat-1][2]!=0.0;
...
/*			if(nav->ssr[obs->sat-1].pbias[obs->code[i]-1] != 0)
        ...
        if (opt->sateph==EPHOPT_B2b) {
			...
			if (sys==SYS_GPS)
				...
				ix=(i==0?CODE_L2I:CODE_L6I);

            P[i]-=(nav->B2bssr[obs->sat].cbias[ix]);
        }
        else if (legacy_dcb) {   /* GAMP-style P1-C1/P2-C2 dcb correction */
            if (obs->code[i]==CODE_L1C) {
                P[i]+=nav->legacy_cbias[obs->sat-1][1];
            }
            else if (obs->code[i]==CODE_L2C||obs->code[i]==CODE_L2X||
                     obs->code[i]==CODE_L2L||obs->code[i]==CODE_L2S) {
                P[i]+=nav->legacy_cbias[obs->sat-1][2];
            }
        }
    ...
    *Lc=*Pc=0.0;
    ...
    if (legacy_dcb&&frq2==1) { /* GAMP-style P1-P2 dcb correction */
        if (P[0]!=0.0) P[0]-=C2*nav->legacy_cbias[obs->sat-1][0];
        if (P[1]!=0.0) P[1]+=C1*nav->legacy_cbias[obs->sat-1][0];
    }
...
/* detect cycle slips before PPP state prediction, as in GAMP detecs() -------*/
static void detecs_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    int i,j;
    ...
    /* GAMP enables GF/MW tests from gamp.cfg and leaves LLI disabled. */
    if (optflag_int(&rtk->opt,"-GAMP_LLICS",0)) detslp_ll(rtk,obs,n);
    if (optflag_int(&rtk->opt,"-GAMP_MWCS",1)) detslp_mw(rtk,obs,n,nav);
    if (optflag_int(&rtk->opt,"-GAMP_GFCS",1)) detslp_gf(rtk,obs,n,nav);
}
...
static void udclk_ppp(rtk_t *rtk)
{
    ...
    gnsisb=gamp_gnsisb(&rtk->opt);

    /* initialize every epoch for GPS receiver clock; keep ISB states as in GAMP gnsisb=1 */
	for (i=0;i<numofsys;i++) {
        if (i==0) {
            ...
            dtr=rtk->sol.dtr[0]+rtk->sol.dtr[i];
        }
        if (fabs(dtr)<1.0E-16) dtr=1.0E-16;
        if (i==0||gnsisb!=1||rtk->x[IC(i,&rtk->opt)]==0.0) {
...
{
    double lam[2];
    double freq1,freq2,ion,g0,g1,dg,dion,sinel,elev,pos[3],dion_tec=0.0,vari_tec=0.0;
    char *p;
    int i,j,gap_resion=GAP_RESION,sat,slip,ionopnoise,ion_const;
    ...
    ionopnoise=gamp_ionopnoise(&rtk->opt);
    ion_const=gamp_ion_const(&rtk->opt);
    ...
    for (i=0;i<n;i++) {
        sat=obs[i].sat;
        j=II(sat,&rtk->opt);
        lam[0]=satwavelen(sat,obs[i].code[0],nav);
        lam[1]=satwavelen(sat,obs[i].code[1],nav);
        if (ionopnoise==3||((ionopnoise==1||ionopnoise==2)&&rtk->x[j]==0.0)) {
            /* initialize ionosphere delay estimates if zero */
            if (ion_const&&nav->nt>0&&
                iontec(obs[i].time,nav,pos,rtk->ssat[sat-1].azel,1,&dion_tec,&vari_tec)) {
                ion=dion_tec*iontec_state_scale(sat,obs[i].code[0],nav);
            }
            else {
                if (obs[i].P[0]==0.0||obs[i].P[1]==0.0||
                    lam[0]==0.0||lam[1]==0.0) {
                    continue;
                }
            ...
            if ((g1=gfmeas_new(obs+i,nav))!=0.0) ion_gf[sat-1]=g1;
            trace(4,"ion init: sat=%d ion=%.4f\n",sat,ion);
        }
        else if ((ionopnoise==1||ionopnoise==2)&&rtk->x[j]!=0.0) {
            /* update variance of delay state */
            if (ionopnoise==1) {
                rtk->P[j+j*rtk->nx]+=SQR(rtk->opt.prn[1])*fabs(rtk->tt);
            }
            else if (ionopnoise==2) {
                slip=rtk->ssat[sat-1].slip[0]||rtk->ssat[sat-1].slip[1];
                if (!slip&&(g1=gfmeas_new(obs+i,nav))!=0.0&&ion_gf[sat-1]!=0.0) {
                    g0=ion_gf[sat-1];
                    ion_gf[sat-1]=g1;
                    ...
                    if (elev>=30.0) {
                        ...
                        rtk->P[j+j*rtk->nx]+=SQR(rtk->opt.prn[1]/(2.0*sinel))*fabs(rtk->tt);
                    }
                }
                else {
                    elev=rtk->ssat[sat-1].azel[1]*R2D;
...
static void uddcb_ppp(rtk_t *rtk)
{
    ...
    for (i=0;i<ND(&rtk->opt);i++) {
        if (rtk->x[ID(i,&rtk->opt)]==0.0) {
            initx(rtk,1E-6,VAR_DCB,ID(i,&rtk->opt));
        }
    }
}
    ...
    for (f=0;f<NF(&rtk->opt);f++) {
        ...
        for (i=k=0;i<n&&i<MAXOBS;i++) {
            ...
            if (rtk->opt.ionoopt==IONOOPT_IFLC) {
                ...
                l=1;
                lam[0]=satwavelen(sat,obs[i].code[0],nav);
                lam[l]=satwavelen(sat,obs[i].code[l],nav);
                lam[f]=satwavelen(sat,obs[i].code[f],nav);
...
/* calculate satellite elevation for state prediction ------------------------*/
static void calElev_ppp(rtk_t *rtk, const obsd_t *obs, int n, const double *rs)
{
    double rr[3],pos[3],r,e[3];
    int i,sat;

    for (i=0;i<MAXSAT;i++) rtk->ssat[i].azel[1]=0.0;

    if (norm(rtk->x,3)>100.0) {
                      ...
                      const nav_t *nav, double *dion, double *var)
{
    ...
    if (opt->ionoopt==IONOOPT_EST) {
        /* Estimated delay follows GAMP: slant L1 ionosphere delay. */
        *dion=x[II(sat,opt)];
        *var=0.0;
        return 1;
    }
                   ...
                   double *azel)
{
    ...
    double var[MAXOBS*2*NFREQ+MAXSAT],dtrp=0.0,dion=0.0,vart=0.0,vari=0.0,dcb,freq;
    double dion_tec=0.0,vari_tec=0.0;
	...
	int i,j,k,sat,sys,nv=0,nx=rtk->nx,stat=1,frq,code,ion_const,ion_tec_ok;
	double res = 0.0;
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        ion_tec_ok=ion_const&&nav->nt>0&&
                   iontec(obs[i].time,nav,pos,azel+i*2,1,&dion_tec,&vari_tec);
        if (ion_tec_ok) dion_tec*=iontec_state_scale(sat,obs[i].code[0],nav);
        /* satellite and receiver antenna model */
		if (opt->posopt[0]) satantpcv(sat,rs+i*6,rr,nav->pcvs+sat-1,dants);
		...
		corr_meas(obs+i,nav,azel+i*2,&rtk->opt,dantr,rs+i*6,e,r/CLIGHT,dants,
        ...
        for (j=0;j<2*NF(opt);j++) {
			...
			switch (sys) {
                ...
                case SYS_CMP: k=2; break;
                case SYS_GAL: k=3; break;
            ...
            if (opt->ionoopt==IONOOPT_EST) {
                ...
                if (ion_const&&ion_tec_ok&&code==1&&frq==1) {
                    int isys=sys_index(sys);
                    if (isys<ND(opt)) {
                        dcb+=rtk->x[ID(isys,opt)];
                        if (H) H[ID(isys,opt)+nx*nv]=1.0;
                    }
                }
            }
            ...
            if (optflag_int(opt,"-GAMP_VARERR",0)) {
                if (sys==SYS_CMP) {
                    if (curprn>=1&&curprn<=5) var[nv]*=100.0;
                }
                if (sys==SYS_QZS) var[nv]*=25.0;
            }
            else if (sys==SYS_GLO&&code==1) {
                var[nv]+=VAR_GLO_IFB;
            }
				
			
            trace(3,"(%d)%s sat=%2d %s%d res=%9.4f sig=%9.4f el=%4.1f\n",post,str,sat,
				  code?"P":"L",frq+1,res,sqrt(var[nv]),azel[1+i*2]*R2D);
            
            if (post == 0){
                trace(2,"satid:%s sat:%d time:%s\n",id,sat,str);
                trace(2, "v[%d]=%f=y(%f)-(r(%f)+cdtr(%f)-CLIGHT*dts(%f)+dtrp(%f)+C(%f)*dion(%f)+dcb(%f)+bias(%f)-shapiro(%f));\n",
                  nv, v[nv], y, r, cdtr, CLIGHT*dts[i * 2], dtrp, C, dion, dcb, bias,gravitationalDelayModel);
                trace(2,"y-r+shapiro=%f\n",y-r+gravitationalDelayModel);
                trace(2,"dts=%.15f \n",dts[i * 2]);
                trace(2,"var=%.15f \n",var[nv]);
                trace(2,"------------------------- \n");
            }
        ...
        if (opt->ionoopt==IONOOPT_EST&&ion_const&&ion_tec_ok) {
            int ii=II(sat,opt);
            if (H) {
                for (k=0;k<nx;k++) H[k+nx*nv]=0.0;
                H[ii+nx*nv]=1.0;
...
static void update_stat(rtk_t *rtk, const obsd_t *obs, int n, int stat)
{
    ...
    if (rtk->sol.stat==SOLQ_FIX) {
        ...
        rtk->sol.dtr[2]=rtk->x[IC(2,opt)]-rtk->x[IC(0,opt)]; /* BDS-GPS */
        rtk->sol.dtr[3]=rtk->x[IC(3,opt)]-rtk->x[IC(0,opt)]; /* GAL-GPS */
...
extern void pppos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    /* prepare elevation angles before ionosphere and ambiguity prediction */
    calElev_ppp(rtk,obs,n,rs);

    /* GAMP detects cycle slips before temporal state prediction. */
    detecs_ppp(rtk,obs,n,nav);
