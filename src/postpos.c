...
static FILE *fp_stec=NULL;      /* output STEC file */
static FILE *fp_vtec=NULL;      /* output VTEC file */
...
/* integer option in pppopt ---------------------------------------------------*/
static int pppopt_int(const prcopt_t *opt, const char *key, int def)
{
    char *p;
    int val=def;
...
/* get path directory ---------------------------------------------------------*/
static void path_dir(const char *path, char *dir)
{
    const char *p1,*p2,*p;
    int n;
...
static void join_path(char *out, int nout, const char *dir, const char *file)
{
    char sep=RTKLIB_FILEPATHSEP;
    int n;

    if (!dir||!*dir||!strcmp(dir,".")) {
        snprintf(out,nout,"%s",file);
        return;
    }
    n=(int)strlen(dir);
    if (strchr(dir,'/')&&!strchr(dir,'\\')) sep='/';
    if (dir[n-1]=='\\'||dir[n-1]=='/') snprintf(out,nout,"%s%s",dir,file);
    else snprintf(out,nout,"%s%c%s",dir,sep,file);
}
/* parse year and day-of-year from input file name ----------------------------*/
static int date_from_input_name(const char *path, int *year, int *doy)
{
    const char *base=path_base(path),*p;
    char yy[3];
    int y,d;
    ...
    for (p=base;*p;p++) {
        if (!isdigit((unsigned char)p[0])||!isdigit((unsigned char)p[1])||
            ...
            !isdigit((unsigned char)p[6])) continue;
        y=(p[0]-'0')*1000+(p[1]-'0')*100+(p[2]-'0')*10+(p[3]-'0');
        d=(p[4]-'0')*100 +(p[5]-'0')*10 +(p[6]-'0');
        if (1980<=y&&y<=2099&&1<=d&&d<=366) {
            *year=y; *doy=d;
            return 1;
        }
    }
    if (strlen(base)>=7&&isdigit((unsigned char)base[4])&&
        isdigit((unsigned char)base[5])&&isdigit((unsigned char)base[6])&&
        (p=strrchr(base,'.'))&&isdigit((unsigned char)p[1])&&
        isdigit((unsigned char)p[2])) {
        d=(base[4]-'0')*100+(base[5]-'0')*10+(base[6]-'0');
        yy[0]=p[1]; yy[1]=p[2]; yy[2]='\0';
        y=atoi(yy);
        y+=y<80?2000:1900;
        if (1<=d&&d<=366) {
            *year=y; *doy=d;
...
/* find first existing file by wildcard --------------------------------------*/
static int find_first_file(const char *pattern, char *path)
{
    char *efiles[16],files[16][MAXSTRPATH];
    int i,n;

    for (i=0;i<16;i++) efiles[i]=files[i];
    n=expath(pattern,efiles,16);
    for (i=0;i<n;i++) {
        if (file_exists(efiles[i])) {
            strcpy(path,efiles[i]);
            return 1;
        }
    }
    return 0;
}
/* add existing file to semicolon-separated list ------------------------------*/
static int append_file(char *list, int nlist, const char *path)
{
    int n;
    ...
    n=(int)strlen(list);
    if (n>0) {
        if (n+1>=nlist) return 0;
        strcat(list,";");
        n++;
    }
    if (n+(int)strlen(path)>=nlist) return 0;
    strcat(list,path);
    return 1;
}
/* find GAMP-like external ionosphere file ------------------------------------*/
static int find_gamp_iono(const char **infile, int n, char *path)
{
    char dir[MAXSTRPATH],sub[MAXSTRPATH],pattern[MAXSTRPATH];
    int year=0,doy=0;

    path[0]='\0';
    if (n<=0||!infile[0]) return 0;
    path_dir(infile[0],dir);

    if (date_from_input_name(infile[0],&year,&doy)) {
        snprintf(sub,sizeof(sub),"AU%04d%03d_v",year,doy);
        join_path(pattern,sizeof(pattern),dir,sub);
        join_path(pattern,sizeof(pattern),pattern,"CODG*");
        if (find_first_file(pattern,path)) return 1;
    }
    join_path(pattern,sizeof(pattern),dir,"CODG*");
    if (find_first_file(pattern,path)) return 1;

    return 0;
}
/* find GAMP-like DCB files ---------------------------------------------------*/
static int find_gamp_dcb(const char **infile, int n, char *path)
{
    char dir[MAXSTRPATH],sub[MAXSTRPATH],cand[MAXSTRPATH],pattern[MAXSTRPATH];
    char p1p2[MAXSTRPATH]="",p1c1[MAXSTRPATH]="",p2c2[MAXSTRPATH]="",bsx[MAXSTRPATH]="";
    int year=0,doy=0;
    ...
    if (date_from_input_name(infile[0],&year,&doy)) {
        snprintf(sub,sizeof(sub),"AU%04d%03d_v",year,doy);
        join_path(cand,sizeof(cand),dir,sub);

        join_path(pattern,sizeof(pattern),cand,"P1P2*.DCB");
        find_first_file(pattern,p1p2);
        join_path(pattern,sizeof(pattern),cand,"P1C1*.DCB");
        find_first_file(pattern,p1c1);
        join_path(pattern,sizeof(pattern),cand,"P2C2*.DCB");
        find_first_file(pattern,p2c2);
        join_path(pattern,sizeof(pattern),cand,"CAS0MGXRAP_*_DCB.BSX");
        find_first_file(pattern,bsx);
    }
    if (!*p1p2) {
        join_path(pattern,sizeof(pattern),dir,"P1P2*.DCB");
        find_first_file(pattern,p1p2);
    }
    if (!*p1c1) {
        join_path(pattern,sizeof(pattern),dir,"P1C1*.DCB");
        find_first_file(pattern,p1c1);
    }
    if (!*p2c2) {
        join_path(pattern,sizeof(pattern),dir,"P2C2*.DCB");
        find_first_file(pattern,p2c2);
    }
    if (!*bsx) {
        join_path(pattern,sizeof(pattern),dir,"CAS0MGXRAP_*_DCB.BSX");
        find_first_file(pattern,bsx);
    }
    append_file(path,MAXSTRPATH,p1p2);
    append_file(path,MAXSTRPATH,p1c1);
    append_file(path,MAXSTRPATH,p2c2);
    append_file(path,MAXSTRPATH,bsx);
    ...
    while ((nobs=inputobs(obs_ptr,rtk->sol.stat,popt))>=0) {
        ...
        if (mode==SOLMODE_SINGLE_DIR) {    /* forward or backward */
            if (!solstatic) {
                ...
                if (fp_stec) outstec(fp_stec,rtk,&navs,rtk->sol.time);
                if (fp_vtec) outvtec(fp_vtec,rtk,&navs,rtk->sol.time);
            }
                   ...
                   const char **infile, const int *index, int n, const char *outfile)
{
    ...
    ion_const=pppopt_int(&popt_,"-ION_CONST",0);
    if (!*fopt_.iono&&popt_.ionoopt==IONOOPT_EST&&ion_const&&
        find_gamp_iono(infile,n,fopt_.iono)) {
        trace(2,"auto ionex file: %s\n",fopt_.iono);
    }
    if (!*fopt_.dcb&&find_gamp_dcb(infile,n,fopt_.dcb)) {
        trace(2,"auto dcb file: %s\n",fopt_.dcb);
    }

    /* Match GAMP's PPP code priority before RINEX observation selection. */
    if (popt_.mode>=PMODE_PPP_KINEMA) {
        setcodepri(SYS_GPS,0,popt_.sateph==EPHOPT_PREC?"PYWC":"CPYW");
        if (popt_.navsys&SYS_GLO) setcodepri(SYS_GLO,0,"PC");
        if (popt_.navsys&SYS_GAL) setcodepri(SYS_GAL,0,"CABXZ");
        if (popt_.navsys&SYS_CMP) setcodepri(SYS_CMP,0,"IQX");
    }

    /* read ionosphere data file */
    if (*fopt_.iono&&(ext=strrchr(fopt_.iono,'.'))) {
        if (strlen(ext)==4&&(ext[3]=='i'||ext[3]=='I'||
                             strcmp(ext,".INX")==0||strcmp(ext,".inx")==0)) {
            reppath(fopt_.iono,path,ts,"","");
            readtec(path,&navs,1);
        }
    }
    /* read erp data */
    ...
    if (!dcb_ok&&*fopt_.dcb) {  /* then check if DCB file specified */
        reppath(fopt_.dcb,path,ts,"","");
    ...
    /* read ocean tide loading parameters */
    if (popt_.mode>PMODE_SINGLE&&*fopt_.blq) {
        readotl(&popt_,fopt_.blq,stas);
    }
    ...
    if (flag&&popt_.mode>=PMODE_PPP_KINEMA&&popt_.ionoopt==IONOOPT_EST) {
        strcpy(path,outfile);
        strcat(path,".stec");
        if (!(fp_stec=fopen(path,"w"))) {
            trace(2,"stec file open error: %s\n",path);
        }
        strcpy(path,outfile);
        strcat(path,".vtec");
        if (!(fp_vtec=fopen(path,"w"))) {
            trace(2,"vtec file open error: %s\n",path);
        }
    }
    /* write header to output file */
